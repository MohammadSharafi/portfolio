...
/**
 * The lighting atlases, at the size this device can afford.
 *
 * A 4096 lightmap and a 2048 aging mask are about 4 MB on disk and roughly
 * 83 MB in GPU memory once decoded — most of a phone's texture budget spent
 * before any of the room's own geometry arrives. The symptom of overrunning it
 * is not a lower frame rate; it is the browser evicting the tab and reloading,
 * which a visitor reads as the site being broken.
 *
 * So the low tier gets half of each dimension, a quarter of the memory, and
 * resolution it could not have resolved anyway: a 4096 atlas over a whole room
 * is near a centimetre per texel, and no phone showing the entire room in
 * 375 pixels can tell that from two.
 *
 * Generated by `scripts/downscale_textures.py`. If they are missing the code
 * falls back to the full-size pair rather than failing — an unlit room would be
 * a much worse answer to "the small file is not there".
 */
const FULL = { lightmap: '/models/room-lightmap.jpg', aging: '/models/room-aging.png' };
const HALF = {
  lightmap: '/models/room-lightmap-half.jpg',
  aging: '/models/room-aging-half.png',
};
...
export function useRoomAsset(): RoomAsset {
  ...
  useEffect(() => {
    ...
    const settle = (found: boolean, intensity = 1, aged = false, paths = FULL) => {
      if (cancelled) return;
      ...
      setAsset({
        url: ROOM,
        lightmap: found ? paths.lightmap : null,
        intensity: found ? intensity : 1,
        aging: found && aged ? paths.aging : null,
        resolved: true,
      });
    };
    ...
    void (async () => {
      try {
        // Half-size on the tier that needs it, but only if the file is really
        // there — a missing downscale must cost memory, never lighting.
        const wantHalf = quality().halfTextures;
        let paths = FULL;
        if (wantHalf && isRealFile(await fetch(HALF.lightmap, { method: 'HEAD' }))) {
          paths = HALF;
        }

        const image = await fetch(paths.lightmap, { method: 'HEAD' });
        ...
        const aged = matches && isRealFile(await fetch(paths.aging, { method: 'HEAD' }));
        settle(matches, intensity ?? 1, aged, paths);
      } catch {
        settle(false);
      }
    })();
