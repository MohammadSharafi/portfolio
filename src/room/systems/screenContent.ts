 ...
 * the whiteboard was diagnosed as needing `mirror-u` and given it, which turned
 * a vertical flip into a 180° rotation. The tell is the letters — upside-down
 * text renders `n` as `u` and `p` as `b` while leaving the left-to-right order
 * of the line alone. Check a line's word order before reaching for a mirror.
 */
...
export function stickyTexture() {
  ...
  // Same wall and same quad rotation as the whiteboard, so the same turn.
  return finish(canvas, 'flip-v');
}
